<template>
  <div>
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
      <el-form :model="formOptions" :rules="rules" ref="">
        <template v-for="(item,index) in fromData">
          ...
          <el-form-item :label="item.label" :key="index" v-if="item.children">
            <component v-model="formOptions[item.prop]" :is="`el-${item.type}`" v-bind="item.attrs">
              <component
...
export default {
  ...
  methods: {
    ...
    //点击取消
    cancel() {
      this.$emit("cancel",this.formOptions);
    },
    //点击确定
    ok() {
      this.$emit("ok",this.formOptions);
    },
  },
  mounted() {
    this.setInitvale()

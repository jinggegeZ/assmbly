    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
      <el-form>
        <template v-for="(item,index) in fromData">
          ...
          <el-form-item :label="item.label"  :key="index" v-if="item.children">
            <component :is="`el-${item.type}`" v-bind="item.attrs" v-model="value">
              <component v-for="(child,idx) in item.children" :key="idx" :is="child.type" :label="child.label" :value="child.value"></component>
            </component>
          </el-form-item>
        </template>
      </el-form>
